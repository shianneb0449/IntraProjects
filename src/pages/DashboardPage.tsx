...
import { FolderKanban, CheckSquare, Activity, AlertCircle, TrendingUp, TrendingDown } from "lucide-react";
import { useNavigate } from "react-router";
import { useData } from "../contexts/DataContext";

const statusColors: Record<string, string> = {
  Active: "#34d399", Planning: "#94a3b8", Draft: "#94a3b8",
  OnHold: "#fbbf24", Completed: "#60a5fa", Cancelled: "#f87171",
};
...
export function DashboardPage() {
  ...
  const statusDist = Object.entries(
    charters.reduce((acc, c) => { acc[c.statusCode] = (acc[c.statusCode] ?? 0) + 1; return acc; }, {} as Record<string, number>)
  ).map(([status, count]) => ({ status, count, color: statusColors[status] ?? "#94a3b8" }));

  const progressData = charters.map(c => ({
    name: c.charterNumber,
    progress: c.taskCount > 0 ? Math.round((c.completedTaskCount / c.taskCount) * 100) : 0,
  }));
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {chartersLoading ? "Loading…" : `${charters.length} charters · live data from API`}
          </p>
        </div>
        <button onClick={() => navigate("/charters")} className="flex items-center gap-2 px-3.5 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors">
          + New Charter
        </button>
      </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          {progressData.length > 0 && (
            <div className="bg-card border border-border rounded-lg p-5">
              <h3 className="text-sm font-semibold text-foreground mb-4" style={{ fontFamily: "'Outfit', sans-serif" }}>Charter progress</h3>
              <ResponsiveContainer width="100%" height={Math.max(160, progressData.length * 36)}>
          ...
          <div className="bg-card border border-border rounded-lg overflow-hidden">
            ...
            {chartersLoading ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {charters.map((c, i) => {
                    const prog = c.taskCount > 0 ? Math.round((c.completedTaskCount / c.taskCount) * 100) : 0;
                    return (
                      <tr
                        key={c.charterId}
                        onClick={() => navigate(`/charters/${c.charterId}`)}
                        className={`hover:bg-white/[0.02] cursor-pointer transition-colors ${i < charters.length - 1 ? "border-b border-border" : ""}`}
                      >
                        <td className="px-4 py-2.5 text-xs font-mono text-muted-foreground">{c.charterNumber}</td>
                        <td className="px-4 py-2.5 text-sm text-foreground max-w-[200px] truncate">{c.title}</td>
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-2">
                            <div className="w-16 h-1.5 rounded-full bg-muted overflow-hidden">
                              <div className={`h-full rounded-full ${pcol(prog)}`} style={{ width: `${prog}%` }} />
                            </div>
                            <span className="text-xs font-mono text-muted-foreground">{prog}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-2.5 text-xs text-muted-foreground font-mono">{c.status}</td>
                      </tr>
                    );
                  })}
                </tbody>
